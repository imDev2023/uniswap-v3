...
Builds #12-#38 are merged; **#39 is in progress** - see Current state. #26-#29 and #33-#39 were scoped in-session and have no issues of their own.
...
🟡 **#39 (creator fee earnings) is reviewed and COMPLETE but UNCOMMITTED** on `build/39-creator-fee-earnings`; its acceptance table is in [`docs/deployments-testnet.md`](docs/deployments-testnet.md). #38 merged as `637282d`.
⚠️ **`377a9f8` on that same branch is NOT #39's work**: the user installed [`evm-security-standards`](contracts/lib/evm-security-standards) (submodule pinned at `e3e0150`, plus Slither/Solhint/Echidna/Medusa configs and `.github/workflows/evm-security.yml`). Merging #39 therefore carries it too. **Its gate reports 8 blocking failures, 4 of them property failures** - the honest starting position, not a regression, and bringing it to zero is #40. Its `lint` job also runs `forge fmt --check`, which is not clean here.
...
✅ **#39 gave settled decision 1 its read side** (`FeeEarningsCard`). Two numbers that must never merge: lifetime **collected** (indexer) and currently **accrued** (chain, simulating `LPLock.collect`). `collect` is permissionless, so a position can hold a year of fees with a lifetime total of zero, and showing only the indexed side tells that creator they earned nothing.
⚠️ **The indexer stores `amount0`/`amount1` VERBATIM and never names the assets** - resolving them needs `token0()` on the pool, which deadlocks a handler (see Indexer honesty). `usePoolTokenOrder` does it client-side. `CLAIM` is `token1` in its own pool, so the non-obvious branch is the live one.

**Creator concentration means two numbers, decided 2026-08-05**: `devAllocation` (granted, the headline) plus `devClaimed` (actually released), shown together. They diverge for the whole 30 days after graduation, which #38 finally saw against real data.
...
⬅️ **Next: commit #39, then #40 = take the `evm-security` gate from 8 blocking failures to 0** (the tooling landed in `377a9f8`; 4 of the 8 are property failures and are the substantive part). After that the critical path is Stage 4's **key protection**, which blocks every public deploy and therefore blocks hosting, which in turn blocks SlowMist's frontend hardening. Stage 3's mobile-wallet gap is the largest user-facing hole.
...
- ⚠️ **`forge fmt` is not clean on this repo.** Running it reformats **12 files nobody touched** - forge 1.7.1 wraps differently from whatever formatted them. #38 reverted that churn by hand rather than ship it inside a deploy ticket. Sweep it in its own commit, or pin the formatter?
- **Two #37 review findings are still deferred**, both cheap and both deliberately kept out of #38: the `ok<T>()` helper at `CreatePage.tsx:80` casts away the types wagmi infers from the as-const ABI, and `function Kv` is byte-identical in three files.
