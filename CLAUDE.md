...
Ticket rhythm (build tickets #12–#21 built the stack; #22–#25 are merged; **Stages 1 and 2 are done. `build/26-mainnet-rpc-probe` (Stage 4, mainnet RPC capability) is BUILT AND REVIEWED but NOT YET MERGED - 2 commits, working tree clean. The next branch is `build/27-<slug>`**; remaining work is Stage 3 (frontend redesign) or more of Stage 4, both in parallel with the audit wait. Tracked as GitHub issues; the map is #1, spec is #11):
...
**Current state:** #12–#25 done and merged to `main` — **the full build sequence is complete**, and the stack is **deployed and validated on testnet 46630**. Build **#24** (Stage 1: frozen curve params in `LaunchCreated`, on-chain `metadataURI`, `token`-indexed trade events, optimizer on) and build **#25** (Stage 2: RPC-first trading) are merged. ✅ **`main` is pushed and in sync with `origin/main` (2026-07-29).** ⚠️ **Build #26 (Stage 4, mainnet RPC probe) is complete and reviewed but sits UNMERGED on `build/26-mainnet-rpc-probe` (2 commits, clean tree) - the user has not yet said whether to merge it.** Addresses, tx hashes and the full end-to-end smoke test are in **`docs/deployments-testnet.md`**; `subgraph/networks.json` and `frontend/.env.local` are wired to them. Contracts are feature-complete and **frozen for audit**; subgraph (#19) in `subgraph/`; frontend (#20/#21/#25) in `frontend/`.
...
**Build #26 closed the one item that could have invalidated the whole approach: the mainnet RPC probe.** See the ✅ bullet below and `docs/rpc-capability.md`. Everything else in this stage (hosting, monitoring, second endpoint, Postgres collation) is still open.
...
- ⚠️ **Still open: a second RPC endpoint has not been sourced.** Only the official public URL is measured, and the state-availability variance above makes a second, more consistent provider more valuable than it first appeared. Rate limits are also uncharacterised (30 concurrent all succeed; sustained probing trips a few HTTP 429s per run).
...
- **Metadata pinning (Stage 3)** — a serverless pin endpoint (a pinning API key cannot live in a Vite bundle), creators bringing their own URI (current v1 behaviour), or a client-safe upload service. Also needs an IPFS gateway choice, a fallback avatar for unpinned/typo'd URIs, and a moderation denylist.
- **`SAFE` must become a REAL multisig before mainnet** — on testnet it is the deployer EOA.
- **Merge `build/26-mainnet-rpc-probe` to `main`?** Built, reviewed, both suites green, 2 commits, clean tree - but the user was asked and has not answered. Nothing depends on it (it adds a script and docs only), so it can also just sit.
- **Source a second mainnet RPC provider (candidate `build/27`)?** Raised by #26's findings rather than pre-planned. Stage 4 always called for two endpoints with failover, but the measured state-availability variance on the official endpoint makes a second, *more consistent* provider more valuable than it looked when scoped. `scripts/rpc-probe.mjs <url>` is ready to evaluate candidates.
