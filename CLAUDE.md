...
- ✅ **RESOLVED 2026-07-29 (build #26) — mainnet 4663 RPC is measured.** Full results in **`docs/rpc-capability.md`**; re-run any time with `node scripts/rpc-probe.mjs <mainnet|testnet|url>` (no deps, and it accepts any URL so it doubles as the provider-evaluation harness). **The "rebuildable cache" assumption HOLDS: mainnet serves `eth_getLogs` from block 0 with no pruning and no block-span cap.** Three findings that change Stage 4 work: (1) **mainnet blocks are 0.100 s**, 2.5x faster than testnet, so the block-ingestor bottleneck that already needs tuning at 0.3 s gets ~3x worse - budget indexer tuning as real work, not a config copy; (2) `eth_getLogs` caps at **10,000 results** (no span cap), so chunking depends on log density, not block count; (3) mainnet historical **state** is retained only ~18.15M blocks (~21.1 days) and it is a moving window, so mainnet fork tests must read the head at runtime exactly as `QuoterV2.t.sol` does. Also: `fromBlock: "earliest"` is **rejected** on both chains (hex only), filter/trace/`eth_syncing` methods are absent, Multicall3 is present on both, and head timestamps track wall clock to 0 s (which independently confirms the Stage 2 lag measurement).
- ⚠️ **Two measurement traps, both of which produced confidently wrong readings before being caught** (recorded in `docs/rpc-capability.md`): `eth_getBalance` returns `0x0` with **no error** on pruned state, so measuring archive depth with it reports "full archive" on a pruned node - **always use `eth_call`**, which errors with `missing trie node`. And a fixed-width log window blows the 10k result cap at every depth on a busy chain, so every row reads ERROR while looking like it was tested.
- ⚠️ **Still open: a second RPC endpoint has not been sourced.** Only the official public URL is measured. Its client string alternates between `linux-arm64` and `linux-amd64`, so it is already a load-balanced pool - but that is one operator behind one DNS name, not independent failover. Rate limits are also uncharacterised (30 concurrent all succeed; one HTTP 429 appeared under heavy probing).
...
⚠️ **Fork tests against 46630 must not fork at `latest`.** The node rejects state at the newest block (`-32000 unsupported block number`) and prunes state after ~5,600 blocks (~28 min). `QuoterV2.t.sol` reads the head at runtime and forks 300 blocks back; a hardcoded block number rots within the hour. **Re-measured 2026-07-29 (#26):** state pruning confirmed at **6,250 blocks (~27 min)**, matching the ~5,600 figure. A plain `eth_call` at `latest` did *not* reproduce the `unsupported block number` rejection, so that part looks specific to how forge forks rather than to bare RPC - the guidance is unchanged either way, since the 27-minute state window alone forces stepping back. **The same applies to mainnet 4663, where the window is ~21.1 days** - longer, but still moving, so still read the head at runtime.
