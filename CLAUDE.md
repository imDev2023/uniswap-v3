...
⚠️ **Build #24 redeployed the whole stack**, so every address above moved and the launches named in this paragraph (`P1ETH`, `SNIPE`, `GRAD`, `RDOGE`, `SMOKE`, `ONEETH`) live on the **superseded** factory `0xE98B99ADD42c550bf40B887Bf07A8f0119a22232`. Their validation records still stand for the behaviour they exercised, but they are not reachable from the current factory, and **the contracts needed re-verifying on Blockscout**: `LaunchpadFactory` is verified (optimizer 200 confirmed in explorer metadata), but `GraduationManager`/`LPLock` **cannot be verified on the testnet explorer at all** — they are internal `CREATE`s from the factory constructor and this self-hosted Blockscout has no `creation_bytecode` for them. Our source is provably right (runtime bytecode differs only in immutable address slots — 80 and 320 bytes exactly). Re-check on mainnet, whose explorer is Blockscout-hosted; details and the fallback option in `docs/deployments-testnet.md`. The current deployment was re-validated end-to-end via launch `META` — see `docs/deployments-testnet.md`.
