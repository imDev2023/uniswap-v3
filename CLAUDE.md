...
⚠️ **`frontend/.env.local` is still on the OLD scheme**, so `npm run build` fails by design with a guard error naming `RPC_UPSTREAM_URL`. That is the feature working. The migration command is in Open decisions; the template it copies from is now correct on disk.
...
⬅️ **Next: BUILD the hosting ticket. The host is DECIDED: Cloudflare Pages, chosen 2026-08-10.** It is the critical path because it serves `/rpc` in production and unblocks SlowMist's frontend hardening and a bug bounty. The decision is taken - do not re-open it - but **nothing has been built or verified**, and the reasoning was proxy economics rather than a measurement: an RPC proxy is high-traffic (the app polls curve state every ~5 s), so free-tier *request* limits are the binding constraint, and Cloudflare's are the most forgiving. ⚠️ **Those tier limits were NOT verified before the choice** and move often. Verify them as step one; if they no longer hold, say so rather than building on the assumption. What the ticket owes: a Pages Function serving `/rpc` from `RPC_UPSTREAM_URL` as a runtime binding, `VITE_RPC_PROXY_PATH=/rpc` in the build environment, and a `_headers` file for HSTS/CSP/SRI. Other live candidates if this is deferred: issue [#22](https://github.com/imDev2023/uniswap-v3/issues/22) (guard or accept the three unguarded protocol-fee functions - the first `contracts/src` change since the testnet deploy) and Stage 3's mobile-wallet gap, the largest user-facing hole.
...
**Stage 4 (infra), still open:** frontend hosting (**host chosen, nothing built** - see Next); monitoring wiring (`scripts/indexer-health.mjs` exists, nothing runs it); Stage-2 RPC fallbacks for the homepage list and curve progress; Blockscout verification of the V3 stack + QuoterV2. SlowMist's frontend hardening (HSTS, CSP, SRI, headers) is blocked on choosing hosting - itemised in [`docs/security-checklist.md`](docs/security-checklist.md#operational-security).
