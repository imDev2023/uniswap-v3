...
Builds #12-#41 are merged (`main` at `c09abe5`); **#42 is committed and reviewed on `build/42-rpc-key-protection` (`73df0a6`), not merged**. #26-#29 and #33-#42 were scoped in-session and have no issues of their own.
...
✅ **#39, #40 and #41 are MERGED** into `main` at `c09abe5`, fast-forward and linear. #40 built the security gate; #41 answered 25 of its 40 attestations and deleted three that conceded the item while still printing `PASS`. Gate reads `pass 45, fail 0, unanswered 15, waived 0` - **verify it rather than trusting this line**. Both are recorded in [`docs/security-checklist.md`](docs/security-checklist.md#the-evm-security-gate-40), with what is still open and what would close each in [the attestation table](docs/security-checklist.md#the-40-attestations-41). Why a green attestation is dangerous at all is under Traps.
✅ **#42 is committed and reviewed at `73df0a6` on `build/42-rpc-key-protection`, NOT merged.** It took the RPC credential out of the bundle *and* out of the wallet - the second leak was the one nobody was tracking, and it is under Traps. ⚠️ Its `.env` half is unfinished; see Open decisions before treating a failing `npm run build` as a regression.
🔴 **NOTHING HAS EVER BEEN PUSHED and CI HAS NEVER RUN.** `origin/main` is `aaf5ed0`, **13 commits** behind local `main`, and `.github/workflows/evm-security.yml` does not exist on GitHub at all - the only workflows there are `forge-std`'s own vendored ones. So there is no red X and there cannot be one until a first push. ⚠️ At that moment the `test` job needs `RPC_MAINNET_ARCHIVE_URL` and `RPC_TESTNET_ARCHIVE_URL` as **repository secrets**, because `contracts/.env` is gitignored and a runner never sees it. Every other job (lint, build, invariants, slither, gate) needs no secrets.
...
**Suites** (verify, do not trust): `forge test` (210, measured 2026-08-09), `cd subgraph && npm test` (36), `cd frontend && npm test` (499, measured 2026-08-10) plus `tsc -b` and `vite build`.
...
⬅️ **Next: merge #42, then pick hosting** - it is the critical path now that key protection is closed in the code, because it serves `/rpc` in production and unblocks SlowMist's frontend hardening and a bug bounty. ⚠️ Choosing it is a *decision*, not a ticket: ask before scoping. The other live candidates are issue [#22](https://github.com/imDev2023/uniswap-v3/issues/22) (guard or accept the three unguarded protocol-fee functions - the first `contracts/src` change since the testnet deploy) and Stage 3's mobile-wallet gap, the largest user-facing hole.
...
**Stage 4 (infra), still open:** frontend hosting; monitoring wiring (`scripts/indexer-health.mjs` exists, nothing runs it); Stage-2 RPC fallbacks for the homepage list and curve progress; Blockscout verification of the V3 stack + QuoterV2. SlowMist's frontend hardening (HSTS, CSP, SRI, headers) is blocked on choosing hosting - itemised in [`docs/security-checklist.md`](docs/security-checklist.md#operational-security).
✅ **Key protection closed in the code (#42).** ⚠️ Its other half is now a hosting task: **production has no `/rpc`**, because Vite serves that path in `dev` and `preview` only. What holds, what does not, and why a proxy protects the key from theft but not the quota from use: [`docs/security-checklist.md`](docs/security-checklist.md#rpc-key-protection).
...
- 🔴 **Push to GitHub?** Never done at any point in this project, so `origin/main` is 13 commits stale and CI has never executed. The first push is also what makes the repository-secrets question real. Ask; never push unprompted.
- 🔴 **`frontend/.env.example` and `.env.local` are the unfinished half of #42**, and neither is reachable from a session: the user's global settings deny `Read(**/.env.*)`, which is correct and should stay. The replacement template was handed over for the user to `cp` in; until `.env.local` moves its key from `VITE_RPC_URL` to `RPC_UPSTREAM_URL` plus `VITE_RPC_PROXY_PATH=/rpc`, **`npm run build` fails by design**. Check whether it was applied before reading a build failure as a regression.
