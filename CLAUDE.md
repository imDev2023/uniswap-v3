...
**Remaining:** (1) **mainnet 4663 deploy** — same runbook, `docs/deploy.md`; (2) **stand up a self-hosted graph-node** (nothing is indexing yet — `subgraph/README.md` lists the stack, and the public RPC's `eth_getLogs` limits are unverified); (3) verify the V3 stack on Blockscout (deployed via `vm.getCode`, so it needs a standard-JSON input from the upstream artifacts).
