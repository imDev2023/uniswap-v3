...
Builds #12-#44 are all merged; **verify `main` with `git log --oneline -1` rather than trusting a SHA written here**. #26-#29 and #33-#44 were scoped in-session and have no issues of their own.
...
⚠️ The deploy build sets `VITE_SUBGRAPH_URL=https://indexer.invalid/...` on purpose. The indexer is local-only, and the default would otherwise put `http://localhost:8100` in a **public** CSP and make every visitor's browser probe their own machine. `.invalid` is RFC 2606 reserved, so the app shows its degradation banner honestly. **Change this the moment a reachable indexer exists.**
...
**Suites** (verify, do not trust): `forge test` (210), `cd subgraph && npm test` (36) both re-measured green on 2026-08-10; `cd frontend && npm test` is **608 in 45 files** as of #44 (2026-08-11), plus `tsc -b` and `vite build`.
...
⬅️ **Next: choose.** #43 is deployed and #44 (the two defects that deployment exposed) is done, so nothing is half-finished. The strongest candidates, in order of what a real-money launch needs: **a reachable indexer** (the deployed site currently has none, so the whole indexed half of the UI is a degradation banner - and it is what `indexer.invalid` is standing in for); **viem request batching** (`http(url, { batch: true })`, which folds a poll cycle into one HTTP request and is the biggest lever on the ~60 tab-hours/day `/rpc` budget); and issue [#22](https://github.com/imDev2023/uniswap-v3/issues/22) (guard or accept the three unguarded protocol-fee functions - the first `contracts/src` change since the testnet deploy) and Stage 3's mobile-wallet gap, the largest user-facing hole.
...
**Stage 4 (infra), still open:** ✅ frontend hosting is **DEPLOYED** (see Current state); ⚠️ but the deployed site has **no reachable indexer**; monitoring wiring (`scripts/indexer-health.mjs` exists, nothing runs it); Stage-2 RPC fallbacks for the homepage list and curve progress; Blockscout verification of the V3 stack + QuoterV2.
...
- 🔴 **A reachable indexer for the deployed site.** Live at `octopus-68a.pages.dev` with `VITE_SUBGRAPH_URL=https://indexer.invalid/...`, so every indexed panel is a degradation banner. Three routes: a fresh Goldsky deploy against the **#38** addresses (the existing `octopus-probe/1.0.0` is from 2026-08-01 and indexes the superseded #24 contracts, so it is useless as-is), exposing the local stack, or leaving it degraded on purpose. Ask before choosing.
...
- 🔴 **Push to GitHub?** Never done at any point in this project, so `origin/main` is 22 commits stale and CI has never executed. The first push is also what makes the repository-secrets question real. Ask; never push unprompted.
