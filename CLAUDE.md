...
Builds #12-#41 are merged (`main` at `c09abe5`); **#42 is in progress on `build/42-rpc-key-protection`**. #26-#29 and #33-#42 were scoped in-session and have no issues of their own.
...
✅ **#41 answered 25 of the gate's 40 attestations and was reviewed the same day** (2026-08-09), committed at `dbae97b` and **merged into `main` on 2026-08-10** (fast-forward, linear). Gate reads `pass 45, fail 0, unanswered 15, waived 0` - verify it rather than trusting this line. No Solidity changed. The review deleted three attestations that conceded the item in their own opening words while still printing `PASS`, and corrected eight enumerations that failed re-derivation from source. What is open, why, and what would close each: [`docs/security-checklist.md`](docs/security-checklist.md#the-40-attestations-41). Why a green attestation is dangerous at all is under Traps.
...
⚠️ #42 also raised vitest's `testTimeout` to 15 s. Idle, no frontend test approaches a second; under concurrent load three render-heavy page tests failed at the 5 s default **every time**, always as timeouts and never as assertions. Reproduce with three `npx vitest run` in parallel before assuming a page test is genuinely broken.
...
⬅️ **Next: choose.** #41 is merged and #42 closed key protection in the code, so the critical path is now **hosting** - it is what serves `/rpc` in production and what unblocks SlowMist's frontend hardening and a bug bounty. The other live candidates are issue [#22](https://github.com/imDev2023/uniswap-v3/issues/22) (guard or accept the three unguarded protocol-fee functions - the first `contracts/src` change since the testnet deploy) and Stage 3's mobile-wallet gap, which is the largest user-facing hole.
...
✅ **Key protection is CLOSED IN THE CODE (#42, 2026-08-10)** and the remaining half is a hosting task, not a code one. The credential moved to `RPC_UPSTREAM_URL` (no `VITE_` prefix, so Vite cannot inline it), the app calls `VITE_RPC_PROXY_PATH` on its own origin, and `npm run build` now **fails** on a credential-shaped URL in the output. ⚠️ **Production still has no `/rpc`** - Vite serves it in `dev` and `preview` only, so a deployed host must serve it or the build must take the `ALLOW_BUNDLED_RPC_CREDENTIAL=1` opt-out. What holds, what does not, and why a proxy is not a boundary against *use*: [`docs/security-checklist.md`](docs/security-checklist.md#rpc-key-protection).
...
- 🔴 **Push to GitHub?** Never done at any point in this project, so `origin/main` is ~10 commits stale and CI has never executed. The first push is also what makes the repository-secrets question real. Ask; never push unprompted.
