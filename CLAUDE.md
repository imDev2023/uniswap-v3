...
Builds #12-#42 are merged (`main` at `2079621`); **#43 is committed on its branch and NOT merged**. #26-#29 and #33-#43 were scoped in-session and have no issues of their own.
...
✅ **#43 (hosting) is MERGED AND DEPLOYED, and verified on the real edge (2026-08-11).** It is live at **https://octopus-68a.pages.dev** (Cloudflare Pages project `octopus`, **direct upload**; `octopus.pages.dev` was taken globally, hence the suffix). `main` fast-forwarded to `31abfa3`. It adds `frontend/functions/rpc.ts` (the Function serving `/rpc`), `frontend/wrangler.jsonc`, a **generated** `dist/_headers`, and `wrangler` as a devDependency.
...
⚠️ **`frontend/.env.local` is gitignored, machine-local, and STILL on the old RPC scheme**, so a bare `npm run build` **fails by design** with a guard error naming `RPC_UPSTREAM_URL`. That is the feature working. Its addresses are the #38 deployment and exist on this machine only; a fresh clone re-derives them from [`docs/deployments-testnet.md`](docs/deployments-testnet.md). Migration command under Open decisions.
...
- 🔴 **Push to GitHub?** Never done at any point in this project, so `origin/main` is 18 commits stale and CI has never executed. The first push is also what makes the repository-secrets question real. Ask; never push unprompted.
