...
- [0004](./docs/adr/0004-managed-subgraph-hosting-on-goldsky.md) - Indexing is hosted, not self-run. ⚠️ **Not what we actually do.** Indexing has been self-hosted on a local graph-node since #27 and the migration was declined again in #38, because stopping the indexer by hand is how the UI's degradation is tested. Goldsky remains a one-line `VITE_SUBGRAPH_URL` change. The ADR is aspirational, not descriptive.
...
- **Launchpad → Indexing**: one-way, event-sourced, and Indexing makes **no on-chain calls at all**. ⚠️ **This is a hard constraint, not a style preference, and #39 proved it by breaking it**: one `token0()` call in a handler deadlocked the whole subgraph, because our RPC prunes state and a historical `eth_call` retries forever while the status stays `healthy`. A handler may only read what the event carries. Everything in the read model is derived, and therefore rebuildable by re-indexing. It consumes four sources, not one: the factory (`LaunchCreated`, `LaunchConfig`), each curve template (`Bought`, `Sold`, `Graduation`, `Graduated`), `LPLock` and `DevVesting`. The manifest in [`subgraph/subgraph.yaml`](./subgraph/subgraph.yaml) is the list; do not maintain a copy of it here.
...
- **Indexing → Trading UI**: analytics only - charts, feeds, positions, rollups. Every surface fed by Indexing must be able to degrade on its own without taking the trade path with it.
  ⚠️ **Narrowed in #37.** A Launch's *terms* - the Dev Allocation, the vesting window, the lock duration, the creator fee share, the permanent-lock choice - are frozen at creation and now read from the **Launchpad**, not from Indexing, even though Indexing carries all of them. Anything frozen at `createLaunch` belongs on the direct-read side by default; Indexing is for what only it knows, which here is the realised `Lock` record, the `FeeCollection` history and everything derived from trades.
  ⚠️ **Widened again in #39.** What a locked position has ACCRUED but not collected is unknowable to Indexing - no event fires until someone collects, and `collect` is permissionless - so it is simulated on the Launchpad side. The position's own `tokenId` moved with it: a panel is not on the chain until every value it reads is.
